      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <form onSubmit={parseMRF} className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {codeChips.map((c) => (
                  <span key={c.code} className="text-xs px-2 py-1 rounded-full border">
                    {c.code} · {c.label}
                    <button type="button" onClick={() => removeCode(c.code)} className="ml-2 opacity-70 hover:opacity-100" aria-label={`Remove ${c.code}`}>×</button>
                  </span>
                ))}
                <button type="button" onClick={() => setCodeChips([])} className="text-xs px-2 py-1 rounded border">Clear all</button>
              </div>
            )}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={exportRowsCSV} disabled={!filteredSortedRows.length} className="px-3 py-2 rounded border">Export rows CSV</button>
            <button type="button" onClick={exportSummaryCSV} disabled={!filteredSortedSummary.length} className="px-3 py-2 rounded border">Export summary CSV</button>
          </div>
        </form>
          ...
          <section className="space-y-3">
            <div className="flex items-end gap-2 flex-wrap">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-2 w-full md:w-auto md:flex md:items-center md:gap-2 md:ml-auto">
                <input value={rowProviderQ} onChange={(e) => setRowProviderQ(e.target.value)} placeholder="Filter provider…" className="border rounded p-2 bg-transparent text-slate-100 placeholder-slate-400" />
                <input value={rowCodeQ} onChange={(e) => setRowCodeQ(e.target.value)} placeholder="Filter code…" className="border rounded p-2 bg-transparent text-slate-100 placeholder-slate-400" />
                <input value={rowMinRate} onChange={(e) => setRowMinRate(e.target.value)} placeholder="Min $" inputMode="decimal" className="border rounded p-2 bg-transparent text-slate-100 placeholder-slate-400" />
                <input value={rowMaxRate} onChange={(e) => setRowMaxRate(e.target.value)} placeholder="Max $" inputMode="decimal" className="border rounded p-2 bg-transparent text-slate-100 placeholder-slate-400" />
                <button type="button" onClick={() => { setRowProviderQ(""); setRowCodeQ(""); setRowMinRate(""); setRowMaxRate(""); }} className="px-3 py-2 rounded border">Reset</button>
              </div>
            ...
            <div className="overflow-x-auto border rounded">
              <table className="min-w-full text-sm">
                <thead className="bg-slate-50 text-slate-900">
                  <tr>
                    <th className="p-2 text-left cursor-pointer" onClick={() => setRowSort("provider_name")}>{headerBtn("Provider", rowSortKey === "provider_name", rowSortDir)}</th>
                    <th className="p-2 text-left cursor-pointer" onClick={() => setRowSort("code")}>{headerBtn("Code", rowSortKey === "code", rowSortDir)}</th>
                    <th className="p-2 text-left cursor-pointer" onClick={() => setRowSort("description")}>{headerBtn("Description", rowSortKey === "description", rowSortDir)}</th>
                    <th className="p-2 text-left cursor-pointer" onClick={() => setRowSort("rate_type")}>{headerBtn("Type", rowSortKey === "rate_type", rowSortDir)}</th>
                    <th className="p-2 text-right cursor-pointer" onClick={() => setRowSort("negotiated_rate")}>{headerBtn("Rate", rowSortKey === "negotiated_rate", rowSortDir)}</th>
                  </tr>
                </thead>
                <tbody>
